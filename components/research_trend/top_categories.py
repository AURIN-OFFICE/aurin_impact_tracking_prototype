"""
Section 2 — Most Active Research Categories (Last 3 Years).

Horizontal bar chart ranking FOR fields by publication volume
...
def render_top_categories_by_volume(
    ...
    st.subheader("Most Active Research Categories (Last 3 Years)")
    st.caption(
        f"FOR fields ranked by total publications in the recent three-year period "
    ...
    counts = (
        df.groupby("for_division")["pub_id"]
        .nunique()
        .rename("count")
        .reset_index()
    )
